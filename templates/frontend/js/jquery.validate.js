...
$j.validator = function( options, form ) {
	this.settings = $j.extend( true, {}, $j.validator.defaults, options );
...
$j.extend($j.validator, {
	...
	prototype: {
		...
		focusInvalid: function() {
			if( this.settings.focusInvalid ) {
				try {
					$j(this.findLastActive() || this.errorList.length && this.errorList[0].element || [])
					.filter(":visible")
					.focus()
		...
		formatAndAdd: function( element, rule ) {
			...
			if ( typeof message == "function" ) {
				...
				message = jQuery.format(message.replace(theregex, '{$j1}'), rule.parameters);
			}			
			this.errorList.push({
				message: message,
				element: element
		...
		previousValue: function(element) {
			return $j.data(element, "previousValue") || $j.data(element, "previousValue", {
				old: null,
				valid: true,
	...
	addMethod: function(name, method, message) {
		...
		$j.validator.messages[name] = message != undefined ? message : $j.validator.messages[name];
		if (method.length < 3) {
	...
	methods: {
				...
				$j.ajax($j.extend(true, {
					...
					success: function(response) {
						...
						if ( valid ) {
							...
							errors[element.name] = $j.isFunction(message) ? message(value) : message;
		...
		equalTo: function(value, element, param) {
			...
			var target = $j(param).unbind(".validate-equalTo").bind("blur.validate-equalTo", function() {
				$j(element).valid();
			});
	...
	if (!jQuery.event.special.focusin && !jQuery.event.special.focusout && document.addEventListener) {
		$j.each({
			...
			function handler(e) {
				e = $j.event.fix(e);
				e.type = fix;
				return $j.event.handle.call(this, e);
			}
		});
	};
	$j.extend($j.fn, {
